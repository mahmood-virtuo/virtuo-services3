...
### Phase 11 — split CSS asset URL correction

Files changed:

- `assets/css/src/core.css`
- Generated `assets/css/bundles/core.min.css`, `assets/css/main.css`, and `assets/css/main.min.css`
- `scripts/build-css.js`
- CSS-split documentation

Root cause and URL strategy:

- Thirteen local `url(...)` occurrences in core still used legacy `../img/...` paths authored for `assets/css/main.css`.
- CleanCSS intentionally builds with `rebase: false`, so moving active output to `assets/css/bundles/` preserved those strings and changed their browser resolution from `/assets/img/...` to the nonexistent `/assets/css/img/...`.
- All split-source local project assets now use root-relative `/assets/img/...` public URLs. External HTTP/HTTPS URLs and the blog-detail SVG fragment URL remain unchanged.
- The CSS builder now rejects relative local URLs and root-relative URLs whose target file does not exist, preventing the same class of regression in future split-source changes.

Audit results:

- All nine editable split sources and all nine generated bundles were scanned.
- `core.css` contains one preserved external CDN URL plus 13 local occurrences covering seven unique assets: five SVG masks and two WebP backgrounds.
- `blog-details.css` contains one preserved fragment-only `url(#emiratesMapGradient)` reference.
- The other seven page-family sources contain no `url(...)` references.
- All seven local target files exist and return HTTP 200 from their corrected root-relative paths.
- No generated bundle contains `/assets/css/img/`, `/assets/css/bundles/img/`, `/assets/css/src/img/`, `/assets/css/bundles/fonts/`, or a remaining relative `../img/`/`../fonts/` reference.
- The original failing `/assets/css/img/images/about_mask_img.svg` string is absent from generated core; `/assets/img/images/about_mask_img.svg` returns 200.

Commands run:

- Complete source/generated `url(...)` inventory and local file-resolution audit
- `node --check scripts/build-css.js`
- `npm run build:css` twice with hashes compared for every generated split and compatibility output
- Local HTTP checks for all nine bundles and all seven split-CSS local assets
- Rendered stylesheet checks on home, About, Contact, three service routes, blog listing, one blog detail, Privacy Policy, and an invalid route
- Lightweight representative-route CSS/image/font resource crawl
- `git diff --stat` and `git diff --check`

Validation results:

- Both CSS builds completed successfully and all generated hashes matched between builds.
- Every representative route returned its expected status and rendered exactly one versioned core bundle followed by the correct single family bundle; none rendered `main.css` or `main.min.css`.
- All nine core/family bundle URLs returned HTTP 200 with CSS content types.
- All split-CSS-originated local asset requests returned 200; no new CSS-originated image/font 404 was found.
- A broader 182-resource local crawl exposed three pre-existing HTML image requests with spaces in their filenames on the Government Relations route. The references and files both predate this phase and were not modified; the existing local PHP router returns 404 for their percent-encoded paths. This is not generated by CSS and remains outside this CSS-only correction.
